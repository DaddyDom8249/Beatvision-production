CREATE TYPE public.generation_run_status AS ENUM ('pending','running','completed','failed','cancelled');
...
CREATE UNIQUE INDEX IF NOT EXISTS generation_runs_project_request_key_uq ON public.generation_runs(project_id, request_key);
CREATE INDEX IF NOT EXISTS generation_runs_project_created_idx ON public.generation_runs(project_id, created_at DESC);
CREATE INDEX IF NOT EXISTS generation_runs_project_action_status_idx ON public.generation_runs(project_id, action, status);
...
CREATE TRIGGER generation_runs_touch_updated_at
  ...
  FOR EACH ROW EXECUTE FUNCTION public.touch_generation_run_updated_at();
